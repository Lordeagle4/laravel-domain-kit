...
it('creates a resource controller with the expected methods', function () {
    ...
    expect($contents)->toContain('public function create(Request $request): Response');
...
it('creates an api resource controller returning json responses', function () {
    $this->artisan('make:domain:controller Orders ApiUserController --resource --api')
        ->assertExitCode(0);
...
it('creates flat action classes and injects them into the resource controller', function () {
    config()->set('domain-kit.controller_actions.style', 'flat');

    $this->artisan('make:domain:controller Users UserController --resource --action')
        ->assertExitCode(0);
...
it('creates nested action classes when configured and injects them', function () {
    ...
    $this->artisan('make:domain:controller Accounts AccountController --resource --action')
        ->assertExitCode(0);
    ...
    expect($controller)->toContain('public function create(Create $createAction, Request $request): Response');
    expect($controller)->toContain('public function update(Update $updateAction, Request $request, int|string $id): Response');
    expect($controller)->toContain('public function destroy(Destroy $destroyAction, int|string $id): Response');
});
